...
def full_scan():
    ...
    nm = nmap.PortScanner()
    nm.scan(ip, '1', '-v') #first we check if the host is up or not
    if (nm[ip].state() == 'down'):
        ...
        print("\n Applying some scripts and generating some logs, plase wait...")
        open_ports = ','.join([str(item) for item in ports])
